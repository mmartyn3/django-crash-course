...
def test_cheese_create_correct_title(client, user):
    ...
    Checks that the web page corresponding to CheeseCreateView contains the
    string Add Cheese."""
    # Authenticate the user
    client.force_login(user)
...
def test_cheese_update(client, user, cheese):
    """ Test if a Cheese updates correctly.
    Send a POST request to CheeseUpdateView with data modifying an existing
    cheese and check redirects."""
    # Authenticate the user
    client.force_login(user)
    # Make a request for our new cheese
    form_data = {
